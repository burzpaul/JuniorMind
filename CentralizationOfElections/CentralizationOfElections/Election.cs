...
namespace CentralizationOfElections
{
    ...
    {
        [TestMethod]
        public void Test_All_Candidates_Are_In_Same_Order ()
        {
            ...
            var votingBallots = new VotingBallot[] { new VotingBallot(state1Voting), new VotingBallot(state2Voting) };

            var expected = new Candidate[] { new Candidate("A", 47), new Candidate("B", 45), new Candidate("C", 43),
                                             new Candidate("D", 41), new Candidate("E", 39) };


            CollectionAssert.AreEqual(expected, Centralize());
        }
    }
}
